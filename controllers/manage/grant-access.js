...
module.exports = function(app) {
	...
	function getCardsFromFailureLog(cb) {
		...
		var query = app.models.FailureLog.query();
		query.select();
	...
	function updateAccess(data, card, cb) {
		...
		async.series([
			...
			function clearAccess(next) {
				...
				app.models.CardLockAccess.query().del().where('card_id', card.id).then(function() {
					next();
				}).catch(next);
			},
			...
			function addAccess(next) {
				...
				app.models.CardLockAccess.createBulk(dataArray, next);
			}
